...
int acpi_pld_fill_usb(struct acpi_ctx *ctx, struct acpi_pld *pld, enum acpi_upc_type type,
		      struct acpi_pld_group *group);

/* Turn PLD structure into a 20 byte ACPI buffer */
int acpi_pld_to_buffer(struct acpi_ctx *ctx, const struct acpi_pld *pld, uint8_t *buf, int buf_len);
