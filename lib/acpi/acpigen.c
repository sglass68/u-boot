...
void acpigen_write_name_string(struct acpi_ctx *ctx, const char *name, const char *string)
{
	acpigen_write_name(ctx, name);
...
static void acpigen_emit_simple_namestring(struct acpi_ctx *ctx, const char *name)
{
	int i;
	char ud[] = "____";
	for (i = 0; i < 4; i++) {
...
static void acpigen_emit_double_namestring(struct acpi_ctx *ctx, const char *name, int dotpos)
{
...
static void acpigen_emit_multi_namestring(struct acpi_ctx *ctx, const char *name)
{
	int count = 0;
...
void acpigen_write_processor_package(struct acpi_ctx *ctx, const char *const name,
...
static int acpigen_write_field_offset(struct acpi_ctx *ctx, u32 offset, u32 current_bit_pos)
{
	u32 diff_bits;
...
static void acpigen_write_field_name(struct acpi_ctx *ctx, const char *name, u32 size)
{
	acpigen_emit_simple_namestring(ctx, name);
 ...
 *	}
 */
int acpigen_write_field(struct acpi_ctx *ctx, const char *name, struct fieldlist *l, size_t count,
			uint flags)
{
	int i;
	...
	for (i = 0; i < count; i++) {
		...
		case NAME_STRING:
			...
			break;
		case OFFSET:
			ret = acpigen_write_field_offset(ctx, l[i].bits, current_bit_pos);
			if (ret)
...
static void __acpigen_write_method(struct acpi_ctx *ctx, const char *name, u8 flags)
{
...
void acpigen_write_method_serialized(struct acpi_ctx *ctx, const char *name, int nargs)
{
...
void acpigen_write_pss_package(struct acpi_ctx *ctx, u32 coreFreq, u32 power, u32 transLat,
			      u32 busmLat, u32 control, u32 status)
{
	acpigen_write_package(ctx, 6);
	acpigen_write_dword(ctx, coreFreq);
	acpigen_write_dword(ctx, power);
	acpigen_write_dword(ctx, transLat);
	acpigen_write_dword(ctx, busmLat);
...
void acpigen_write_cst_package_entry(struct acpi_ctx *ctx, struct acpi_cstate *cstate)
{
...
void acpigen_write_csd_package(struct acpi_ctx *ctx, u32 domain, u32 numprocs, CSD_coord coordtype,
	u32 index)
{
...
void acpigen_write_tss_package(struct acpi_ctx *ctx, int entries, struct acpi_tstate *tstate_list)
{
	/*
...
void acpigen_write_mem32fixed(struct acpi_ctx *ctx, int readwrite, u32 base, u32 size)
{
	/*
...
static void acpigen_write_register(struct acpi_ctx *ctx, const struct acpi_gen_regaddr *addr)
{
	...
	acpigen_emit_byte(ctx, addr->space_id);	/* Address Space ID */
	acpigen_emit_byte(ctx, addr->bit_width);	/* Register Bit Width */
	acpigen_emit_byte(ctx, addr->bit_offset);	/* Register Bit Offset */
	acpigen_emit_byte(ctx, addr->access_size);	/* Register Access Size */
	acpigen_emit_dword(ctx, addr->addrl);	/* Register Address Low */
	acpigen_emit_dword(ctx, addr->addrh);	/* Register Address High */
}
 ...
 */
void acpigen_write_power_res(struct acpi_ctx *ctx, const char *name, u8 level, u16 order,
...
void acpigen_write_return_byte_buffer(struct acpi_ctx *ctx, u8 *arr, size_t size)
{
	acpigen_emit_byte(ctx, RETURN_OP);
 ...
 * }
 */
int acpigen_write_dsm_uuid_arr(struct acpi_ctx *ctx, struct dsm_uuid *ids, size_t count)
{
	size_t i;
	int ret;
...
int acpigen_write_cppc_package(struct acpi_ctx *ctx, const struct cppc_config *config)
{
	u32 i;
	u32 max;
...
int __weak acpigen_soc_clear_tx_gpio(struct acpi_ctx *ctx, unsigned int gpio_num)
{
	log_err("not implemented\n");
